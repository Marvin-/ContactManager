...
describe 'the person view', type: :feature do
  ...
  describe 'email addresses' do
    before(:each) do 
      person.email_addresses.create(address: "email1")
      person.email_addresses.create(address: "email2")
      visit person_path(person)
    end
    ...
    it 'adds a new email address' do 
      ...
      page.fill_in('Address', with: 'example@email.com')
      page.click_button('Save')
      expect(current_url).to include("#{person.id}")
    ...
    it 'edits an email address' do 
      ...
      first(:link, 'edit').click
      page.fill_in('Address', with: 'email2')
      page.click_button('Save')
      expect(current_path).to eq(person_path(person))
    ...
    it 'has link to delete email addresses' do 
      person.email_addresses.each do |email|
